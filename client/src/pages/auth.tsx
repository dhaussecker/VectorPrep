    ...
    try {
      ...
      if (msg.includes("provider") || msg.includes("not enabled") || msg.includes("400")) {
        toast({
          ...
          description: "Enable Google in your Supabase dashboard → Auth → Providers.",
          variant: "destructive",
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center relative overflow-hidden">
      <style>{`
        ...
        .btn-press:active { transform: translateY(3px) !important; box-shadow: 0 1px 0 0 hsl(var(--foreground)/.3) !important; }
      `}</style>
      ...
      {floaters.map((f, i) => (
        <div
          key={i}
          className="absolute font-mono font-black text-foreground select-none pointer-events-none"
          style={{
            fontSize: `${f.size}rem`,
            top: `${f.top}%`,
            left: `${f.left}%`,
      ...
      <div className="auth-card relative z-10 w-full max-w-[340px] px-5">
        ...
        <div className="flex flex-col items-center gap-3 mb-8">
          ...
          <div className="text-center">
            <h1 className="text-xl font-black text-foreground font-mono tracking-tight">OnQuest</h1>
            <p className="text-muted-foreground text-xs mt-1 font-mono">Ace your exams. Earn every point.</p>
          </div>
        </div>
        ...
        <button
          ...
          className="btn-press w-full flex items-center justify-center gap-3 py-3.5 px-6 rounded-2xl bg-card border-2 border-foreground shadow-hard transition-all font-semibold text-foreground text-sm disabled:opacity-60 mb-4"
        >
          {googlePending ? (
        ...
        {/* Toggle */}
        <p className="text-center mt-5 text-muted-foreground text-xs font-mono">
